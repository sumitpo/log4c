	     ...
	     const char *fmt, ...)
{
	...
	if (use_color) {
		fprintf(stderr, "%s [%s%s%05d%s] %s:%d %s(): ", time_str, color,
			level_name, getpid(), LOG_COLOR_RESET, filename, line,
			func);
	} else {
		fprintf(stderr, "%s [%s%05d] %s:%d %s(): ", time_str,
			level_name, getpid(), filename, line, func);
	}
